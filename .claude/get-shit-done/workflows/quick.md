...
```bash
INIT=$(node "/home/mgerasolo/Dev/the-keep/.claude/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "/home/mgerasolo/Dev/the-keep/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
